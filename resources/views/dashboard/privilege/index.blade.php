                            ...
                            <div class="row align-items-center">
                                ...
                                <div class="col-xl-4 text-lg-right my-1">
                                    @if (Auth::user()->can('Add Privilege'))
                                        <a href="{{ route('privilege.create') }}" class="btn btn-primary btn-lg fw-bold">
                                    ...
                                    <tbody>
                                        @foreach ($privilegegroups as $index => $privilegegroup)
                                            <tr>
                                                ...
                                                <td>
                                                    {{ $privilegegroup->pg_nama }}
                                                </td>
                                                <td>
                                                    @foreach ($privilegegroup->privilege as $key => $privilege)
                                                        {{ $privilege->p_nama }}
                                                        @if ($key+1 < count($privilegegroup->privilege))
                                                            ,
                                                        @endif
                                                    @endforeach
                                                </td>
                                                <td class="text-center column_action">
                                                    <div class="btn-group" role="group">
                                                        @if (Auth::user()->can('Edit Privilege'))
                                                            <a href='{{ route('privilege.edit', $privilegegroup->pg_id) }}' class="btn btn-sm btn-icon btn-bg-light btn-icon-success fw-bold" title="Ubah">
                                                                <i class="ri-pencil-fill"></i>
                                                            </a>  
                                                        @endif
                                                        @if (Auth::user()->can('Delete Privilege'))
                                                            <button type="button" onclick="deleteModal({{ $privilegegroup->pg_id }})" class="btn btn-sm btn-icon btn-bg-light btn-icon-success fw-bold" title="Hapus">
...
@section('js')
    ...
    <script>
        function deleteModal(id) {
            var urldelete = "{{ route('privilege.destroy', ':param') }}";
